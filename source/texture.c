...
int textureLoadBuffer(texture_s* t, u32* buffer, int width, int height, u32 param, int mipmap)
{
	...
	if(!t->data){return -3;}
	tileImage8((u8*)buffer, (u8*)t->data, t->width, t->height);
	...
	for(level = 0; level < mipmap; level++)
	{
		...
		tileImage8((u8*)buffer, (u8*)&t->data[offset], w, h);
		offset += w*h;
		w /= 2; h /= 2;
	}
...
void textureBind(texture_s* t, GPU_TEXUNIT unit)
{
	...
	GPU_SetTexture(unit, (u32*)osConvertVirtToPhys(t->data), t->width, t->height, t->param, t->format);
